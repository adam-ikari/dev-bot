...
"""
    ...
    def build_execution_prompt(self, memory_summary: str = "") -> str:
        ...
        phase_context = f"""
## 工作任务
分析项目状态，识别需要完成的工作，制定执行计划，并在本次调用中完成所有工作。

## 执行要求
1. 一次性完成：在本次调用中完成所有相关工作，不要分步请求
2. 自主决策：根据项目实际情况，自主决定要完成什么工作
3. 详细记录：记录工作目标、执行过程、操作结果
4. 评估完成度：0-100% 评估工作完成程度
5. 功能验证：完成的功能必须经过测试验证，确保没有问题
6. 明确结论：完成后输出 [进入复盘] 让复盘阶段评估你的工作
...
## 重要提示
- 你应该在一次调用中完成所有工作，不要多次请求
- 完成工作后，输出 [进入复盘] 让复盘阶段评估你的工作
- 复盘阶段会判断是否需要继续执行或功能已经完成
- 不要输出 [工作完成] 或 [继续执行]，这些由复盘阶段判断
"""
